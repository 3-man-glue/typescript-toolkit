...
export class Actor implements Identity {
  ...
  constructor(id: string, alias: string, externalId?: string) {
    if (!id) { throw new IdentityException('Invalid ID. ID must not be an empty string.') }
    if (!alias) { throw new IdentityException('Invalid alias. Alias must not be an empty string.') }
